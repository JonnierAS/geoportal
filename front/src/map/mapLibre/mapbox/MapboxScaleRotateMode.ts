import * as Constants from '@/map/mapLibre/mapbox/mapboxGlLocalFiles/constants';
import doubleClickZoom from '@/map/mapLibre/mapbox/mapboxGlLocalFiles/double_click_zoom';
import createSupplementaryPoints from '@/map/mapLibre/mapbox/mapboxGlLocalFiles/create_supplementary_points';
import * as CommonSelectors from '@/map/mapLibre/mapbox/mapboxGlLocalFiles/common_selectors';
import moveFeatures from '@/map/mapLibre/mapbox/mapboxGlLocalFiles/move_features';
...
function parseSRCenter(value: any, defaultSRCenter = SRCenter.Center) {
 ...
 */
SRMode.onSetup = function (opts: any) {
  ...
  const addImageIfNotExists = (map: any, id: any, src: any) => {
...
SRMode.toDisplayFeatures = function (state: any, geojson: any, push: any) {
...
SRMode.pathsToCoordinates = function (featureId: any, paths: any) {
  return paths.map((coord_path: any) => {
    return { feature_id: featureId, coord_path };
  });
};

SRMode.computeBisectrix = function (points: any) {
...
SRMode.createRotationPoints = function (state: any, geojson: any, suppPoints: any) {
  const { type, coordinates } = geojson.geometry;
  const featureId = geojson.properties && geojson.properties.id;

  let rotationWidgets: any = [];
  if (
    type === Constants.geojsonTypes.POINT ||
...
SRMode.startDragging = function (state: any, e: any) {
  this.map.dragPan.disable();
  state.canDragMove = true;
  state.dragMoveLocation = e.lngLat;
};

SRMode.stopDragging = function (state: any) {
...
SRMode.onTouchStart = SRMode.onMouseDown = function (state: any, e: any) {
...
SRMode.onVertex = function (state: any, e: any) {
...
SRMode.onRotatePoint = function (state: any, e: any) {
  // convert internal MapboxDraw feature to valid GeoJSON:
  this.computeAxes(state, state.feature.toGeoJSON());
...
SRMode.onFeature = function (state: any, e: any) {
  state.selectedCoordPaths = [];
  this.startDragging(state, e);
};

SRMode.coordinateIndex = function (coordPaths: any) {
...
SRMode.computeRotationCenter = function (state: any, polygon: any) {
  var center0 = turf.center(polygon);
  return center0;
};

SRMode.computeAxes = function (state: any, polygon: any) {
  ...
  if (polygon.geometry.type === Constants.geojsonTypes.POLYGON)
    ...
    let temp: any = [];
    polygon.geometry.coordinates.forEach((c: any) => {
      c.forEach((c2: any) => {
        c2.forEach((c3: any) => {
          temp.push(c3);
        });
...
SRMode.onDrag = function (state: any, e: any) {
...
SRMode.dragRotatePoint = function (state: any, e: any, delta: any) {
...
SRMode.dragScalePoint = function (state: any, e: any, delta: any) {
...
SRMode.dragFeature = function (state: any, e: any, delta: any) {
...
SRMode.onMouseOut = function (state: any) {
...
SRMode.onTouchEnd = SRMode.onMouseUp = function (state: any) {
  if (state?.dragMoving) {
    this.fireUpdate();
  }
...
SRMode.clickActiveFeature = function (state: any) {
  state.selectedCoordPaths = [];
  this.clearSelectedCoordinates();
  state.feature.changed();
};

SRMode.onClick = function (state: any, e: any) {
...
SRMode.clickNoTarget = function (state: any, e: any) {
  if (state?.canSelectFeatures) this.changeMode(Constants.modes.SIMPLE_SELECT);
};

SRMode.clickInactive = function (state: any, e: any) {
